    ...
    while True:
        try:
            ...
            with Session(engine) as session:
                first = session.execute(select(Progress).order_by(Progress.date.desc())).scalars().first()
            ...
            clear_screen()
            print(f"Current progress: {progress_value * 100:.2f}%")
            print(f"Current pace: {pace}%/hour / {pace / 60}%/minute")
            print(f"Migration completed by {1 / pace * (1 - progress_value) if pace > 0 else "N/A"}")
